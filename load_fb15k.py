...
class TrainSet(Dataset):
    ...
    def load_text(self):
        raw_data = pd.read_csv('./data/fb15k/freebase_mtr100_mte100-train.txt', sep='\t', header=None,
...
class TestSet(Dataset):
    ...
    def load_text(self):
        raw_data = pd.read_csv('./data/fb15k/freebase_mtr100_mte100-test.txt', sep='\t', header=None,
                               names=['head', 'relation', 'tail'],
                               keep_default_na=False, encoding='utf-8')
        raw_data = raw_data.applymap(lambda x: x.strip())
